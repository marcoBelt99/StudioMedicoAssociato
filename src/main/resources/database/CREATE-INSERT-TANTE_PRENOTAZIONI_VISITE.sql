...
CREATE TABLE IF NOT EXISTS utenti (
    ...
    attivo BOOLEAN DEFAULT TRUE, -- Per la cancellazione logica
...
CREATE TABLE IF NOT EXISTS medici (
    ...
    matricola VARCHAR(20) NOT NULL UNIQUE, -- vincolo di unicita'
...
CREATE TABLE IF NOT EXISTS pazienti (
    id_anagrafica INT PRIMARY KEY,
    codice_fiscale VARCHAR(16) NOT NULL UNIQUE, -- vincolo di unicità
...
CREATE TABLE IF NOT EXISTS visite (
    ...
    id_anagrafica INT NOT NULL,
    id_prestazione INT,
    CONSTRAINT fk_visite_anagrafiche FOREIGN KEY (id_anagrafica) REFERENCES anagrafiche(id_anagrafica) -- id anagrafica del medico
    ON UPDATE CASCADE
    ON DELETE CASCADE,
    CONSTRAINT fk_visite_prestazioni FOREIGN KEY (id_prestazione) REFERENCES prestazioni(id_prestazione) -- dettaglio della visita (cioè prestazione)
    ON UPDATE CASCADE
    ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS prenotazioni (
    ...
    id_visita INT NOT NULL,
    id_anagrafica INT NOT NULL,
    deleted BOOLEAN DEFAULT FALSE, -- per cancellazione logica
    CONSTRAINT fk_prenotazioni_visite FOREIGN KEY (id_visita) REFERENCES visite(id_visita) -- visita a cui fa riferimento la prenotazione
    ON UPDATE CASCADE 
    ON DELETE CASCADE,
    CONSTRAINT fk_prenotazioni_anagrafiche FOREIGN KEY (id_anagrafica) REFERENCES anagrafiche(id_anagrafica) -- id anagrafica del paziente
    ON UPDATE CASCADE
    ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS esiti (
    ...
    descrizione_referto VARCHAR(255), -- fare un TEXT probabilmente
...
-- VIEW anagrafiche_medici
CREATE VIEW public.anagrafiche_medici
AS
SELECT "anagrafiche"."id_anagrafica", "anagrafiche"."cognome", "anagrafiche"."nome", "anagrafiche"."data_nascita",
       "anagrafiche"."genere", "medici"."matricola", "medici"."specializzazione"
FROM "anagrafiche"
         JOIN "medici" ON "medici"."id_anagrafica" = "anagrafiche"."id_anagrafica"
order by "anagrafiche"."id_anagrafica";

ALTER TABLE public.anagrafiche_medici
    OWNER TO postgres;
COMMENT ON VIEW public.anagrafiche_medici
    IS 'Di tutte le anagrafiche presenti a database, recupera solamente quelle dei medici.';

-- VIEW anagrafiche_pazienti
CREATE VIEW public.anagrafiche_pazienti
AS
SELECT "anagrafiche"."id_anagrafica",  "anagrafiche"."cognome", "anagrafiche"."nome",
       "anagrafiche"."data_nascita", "anagrafiche"."genere",
       "pazienti"."codice_fiscale", "pazienti"."telefono",  "pazienti"."email",
       "pazienti"."residenza"
FROM "anagrafiche"
         JOIN "pazienti" ON "pazienti"."id_anagrafica" = "anagrafiche"."id_anagrafica"
ORDER BY "anagrafiche"."id_anagrafica";

;

ALTER TABLE public.anagrafiche_pazienti
    OWNER TO postgres;
COMMENT ON VIEW public.anagrafiche_pazienti
    IS 'Di tutte le anagrafiche, recupera solo quelle relative ai pazienti.';

-- VIEW anagrafiche_infermieri
CREATE VIEW public.anagrafiche_infermieri
AS
SELECT "anagrafiche"."id_anagrafica", "anagrafiche"."cognome", "anagrafiche"."nome", "anagrafiche"."data_nascita",
       "anagrafiche"."genere", "infermieri"."num_cartellino"
FROM "anagrafiche"
         JOIN "infermieri" ON "infermieri"."id_anagrafica" = "anagrafiche"."id_anagrafica"
order by "anagrafiche"."id_anagrafica";

ALTER TABLE public.anagrafiche_infermieri
    OWNER TO postgres;
COMMENT ON VIEW public.anagrafiche_infermieri
    IS 'Di tutte le anagrafiche considera solo quelle relative agli infermieri.';


-- select * from anagrafiche_medici;
-- select * from anagrafiche_pazienti;
-- select * from anagrafiche_infermieri;